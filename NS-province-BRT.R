...
load("D:/CHID regional NS BRT/NS_BRT_Rproject/data_pack_NS.RData")

#pred_abs_2011<- brick("D:/CHID regional NS BRT/prediction dataset/abs2011_250m_NS.grd")
...
#create column in datcombo with management unit ID, as a factor:
datcombo$managemnt_unit<-as.factor(extract(pred_abs_2011$management_unit,datcombo[,c("X","Y")]))
...
datcombo$watNS <- as.factor(datcombo$watNS)
...
start_time<-Sys.time()
brt1<- brt_blocks(data=datcombo,pred.variables = pred.variables, lr=0.01,tc=3, output.folder = "D:/CHID regional NS BRT/BRT_outputs/entire_province/full_model_new/", blocks=sp_block_full, save.points.shp = TRUE)  
end_time<-Sys.time()
end_time-start_time
...
start_time<-Sys.time()
brt2<- brt_blocks(data=datcombo,pred.variables = pred.variables2, lr=0.01,tc=3, output.folder = "D:/CHID regional NS BRT/BRT_outputs/entire_province/selected_scales_new/", blocks=sp_block_select, save.points.shp = TRUE)  
end_time<-Sys.time()
end_time-start_time
...
start_time<-Sys.time()
brt3<- brt_blocks(data=datcombo,pred.variables = pred.variables3, lr=0.00001,tc=2, output.folder = "D:/CHID regional NS BRT/BRT_outputs/entire_province/cell_level_new/", blocks=sp_block_cell, save.points.shp = FALSE)  
end_time<-Sys.time()
end_time-start_time
...
start_time<-Sys.time()
brt4<- brt_blocks(data=datcombo,pred.variables = pred.variables4, lr=0.01,tc=3, output.folder = "D:/CHID regional NS BRT/BRT_outputs/entire_province/GFsigma250m_new/", blocks=sp_block_GF250, save.points.shp = TRUE)  
end_time<-Sys.time()
end_time-start_time
...
start_time<-Sys.time()
brt5<- brt_blocks(data=datcombo,pred.variables = pred.variables5, lr=0.01,tc=3, output.folder = "D://CHID regional NS BRT/BRT_outputs/entire_province/GFsigma750m_new/", blocks=sp_block_GF750, save.points.shp = TRUE)  
end_time<-Sys.time()
end_time-start_time

save.image("D:/CHID regional NS BRT/BRT_outputs/entire_province/outputs.RData")
...
start_time <- Sys.time()
confintbrt5<-boot_brt(datcombo,brt5,sp_block_GF750,pred_abs_2011,nsamples=250,output.folder = "D:/CHID regional NS BRT/BRT_outputs/entire_province/GFsigma750m_new/confint/")
end_time <- Sys.time()
end_time - start_time
...
upper<-raster("D://CHID regional NS BRT/BRT_outputs/entire_province/GFsigma750m_new/confint/confint_upper.tif")
...
lower<-raster("D://CHID regional NS BRT/BRT_outputs/entire_province/GFsigma750m_new/confint/confint_lower.tif")
...
writeRaster(mess1, filename="D://CHID regional NS BRT/BRT_outputs/entire_province/GFsigma250m/mess_brt_predsGF250m.tif", format="GTiff",overwrite=TRUE)
